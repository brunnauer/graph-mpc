...
class AddShare {
   ...
   public:
    AddShare() = default;
    explicit AddShare(R value) : value_{value} {}

    void randomize(emp::PRG &prg) { prg.random_data(value_.data(), sizeof(R)); }
    ...
    // what is "friend"?
    friend AddShare<R> operator+(AddShare<R> lhs, const AddShare<R> &rhs) {
        lhs += rhs;
        return lhs;
    }
