...
class PiKProtocol : public ProtocolDef {
   public:
    PiKProtocol(Party id, RandomGenerators &rngs, std::shared_ptr<io::NetIOMP> network, size_t n, size_t n_bits, size_t n_vertices, size_t n_iterations,
    ...
    virtual void pre_mp_preprocessing(MPPreprocessing &preproc) {
        deduplication_preprocess(id, rngs, network, n, n_bits, preproc, recv_shuffle, recv_mul, ssd);
    }
    ...
    virtual void pre_mp_evaluation(MPPreprocessing &preproc, Graph &g) { deduplication_evaluate(id, rngs, network, n, preproc, g, ssd); }
