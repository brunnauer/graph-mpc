...
void deduplication_preprocess(Party id, RandomGenerators &rngs, std::shared_ptr<io::NetIOMP> network, size_t n, size_t n_bits, MPPreprocessing &preproc,
    ...
    clip::equals_zero_preprocess(id, rngs, network, n - 1, preproc, recv);  // src eqz
    clip::equals_zero_preprocess(id, rngs, network, n - 1, preproc, recv);  // dst eqz
    mul::preprocess(id, rngs, network, n - 1, preproc, recv);               // Multiplication
    mul::preprocess(id, rngs, network, n - 1, preproc, recv);               // B2A
...
void deduplication_evaluate(Party id, RandomGenerators &rngs, std::shared_ptr<io::NetIOMP> network, size_t n, MPPreprocessing &preproc, Graph &g) {
    Permutation perm = sort::get_sort_evaluate(id, rngs, network, n, preproc, g.dst_order_bits);
    preproc.dst_order = perm;

    for (size_t i = 0; i < g.src_bits().size(); ++i) {
        perm = sort::sort_iteration_evaluate(id, rngs, network, n, perm, preproc, g.src_bits()[i]);
    }
