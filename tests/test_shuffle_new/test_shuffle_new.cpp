...
void test_shuffle(const bpo::variables_map &opts) {
    ...
    std::vector<Row> shuffle_share = shuffle(conf, share, perm_share, true);
    std::vector<Row> repeat_share = shuffle(conf, share, perm_share, false);
    std::vector<Row> unshuffle_share = unshuffle(conf, repeat_share, perm_share);
    PermShare perm_share_new = get_shuffle(conf);
    std::vector<Row> new_shuffle_share = shuffle(conf, share, perm_share_new, true);
