...
void test_sw_perm(Party id, RandomGenerators &rngs, std::shared_ptr<NetworkInterface> network, size_t n, size_t BLOCK_SIZE) {
    ...
    Permutation sort_share = sort::get_sort(id, rngs, network, n, bit_shares);
    auto sorted_input_share = permute::apply_perm(id, rngs, network, n, sort_share, input_share);
    ...
    Permutation reverse_sort_share = sort::get_sort(id, rngs, network, n, bit_shares);

    /* Preprocessing */
    StatsPoint start_pre(*network);
    auto preproc = permute::switch_perm_preprocess(id, rngs, network, n);
    StatsPoint end_pre(*network);

    auto preproc_1 = permute::switch_perm_preprocess(id, rngs, network, n);
    ...
    auto reverse_sorted_input_share = permute::switch_perm_evaluate(id, rngs, network, n, sort_share, reverse_sort_share, preproc, sorted_input_share);
    StatsPoint end_online(*network);

    auto inverse_share = permute::switch_perm_evaluate(id, rngs, network, n, reverse_sort_share, sort_share, preproc_1, reverse_sorted_input_share);
    ...
    auto sorted_vector = share::reveal_vec(id, network, sorted_input_share);
    ...
    if (id != D) {
        ...
        auto reverse_sorted = share::reveal_vec(id, network, reverse_sorted_input_share);
        ...
        auto inverse = share::reveal_vec(id, network, inverse_share);
