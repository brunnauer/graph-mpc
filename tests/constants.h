...
constexpr const size_t mp_comm_pre(size_t n, size_t d) {
    return 2 * sort_comm_pre(n, 33) + sort_iteration_comm_pre(n) + apply_perm_comm_pre(n) + d * 4 * switch_perm_comm_pre(n);
}
constexpr const size_t bfs_comm_pre(size_t n, size_t d) {
    return 2 * sort_comm_pre(n, 33) + sort_iteration_comm_pre(n) + apply_perm_comm_pre(n) + d * 4 * switch_perm_comm_pre(n) + eqz_comm_pre(n) + B2A_comm_pre(n);
}

constexpr const size_t pi_k_comm_pre(size_t n, size_t d) {
    return deduplication_comm_pre(n, 32) + 2 * sort_comm_pre(n, 33 + 1) + sort_iteration_comm_pre(n) + apply_perm_comm_pre(n) + d * 4 * switch_perm_comm_pre(n);
}
...
constexpr const size_t mp_comm_online(size_t n, size_t d) {
    return 2 * sort_comm_online(n, 33) + sort_iteration_comm_online(n) + apply_perm_comm_online(n) + d * 4 * switch_perm_comm_online(n);
}
