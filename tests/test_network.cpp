...
void setupExecution(const bpo::variables_map &opts, size_t &pid, size_t &nP, size_t &repeat, size_t &threads, size_t &shuffle_num, size_t &nodes,
    ...
    if (opts["localhost"].as<bool>()) {
        network = std::make_shared<NetworkInterface>((Party)pid, (int)nP, port, (char **)nullptr, certificate_path, private_key_path, trusted_cert_path, true,
                                                     1000000);
    } else {
        ...
        network =
            std::make_shared<NetworkInterface>((Party)pid, (int)nP, port, ip.data(), certificate_path, private_key_path, trusted_cert_path, false, 1000000);
    }
}
